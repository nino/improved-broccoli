...
(defpackage :attack-of-the-robots
  (:use cl arrow-macros))

(in-package :attack-of-the-robots)
...
(defun robots ()
  (loop named main
        ...
        then (progn (format t "~%qwe/asd/zxc to move, (t)eleport, (l)eave:")
                    (force-output)
                    (let* ((choice (read))
                           (dir (assoc choice directions)))
                      (cond (dir (+ pos (rest dir)))
                            ((eq 't choice) (random 1024))
                            ((eq 'l choice) (return-from main 'bye))
                            (t pos))))
